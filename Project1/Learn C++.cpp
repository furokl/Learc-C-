...
* Пример обычной проверки
* if (string[index] == ch)
*	return index;
* 
* return -1; // Если совпадений не найдено, то возвращаем -1
* 
*	1. Не понятно: -1 - это ошибка или корректное возвращаемое значение?
*		(Необходимо видеть код функции)
*	2. Функции возвращает одно значение. А что если хотим вернуть результат выполнения
*		функции и код ошибки? (Запихать код ошибки по ссылке)
*	3. Когда много кода, нужно постоянно проверять коды возврата.
*	4. Конструкторы не умеют возвращать код состояния, а если передадим по ссылке
*		все равно придется лечить последствия и проверять ошибки.
* 
* 
* @.'Исключения' - Обеспечивает механизм, позволяющий отделить обработку ошибок или других
*		исключительных обстоятельств от общего потока выполнения кода.
* 
*	! Не выполняет неявных преобразований
*	! catch срабатывает мгновенно
*	! catch-all - обработка всех типов исключений: catch (...)
*	! Используйте std::unique_ptr для очистки памяти
*	! Не используйте исключения в деструкторах
* 
* 'Когда применять?'
*	Обрабатываемая ошибка возникает редко
*	Ошибка является серъезной, и выполнение программы не может продолжаться без её обработки
...
*
* throw		- генерация исключений (код ошибки)
* try		- поиск исключений (наблюдатель в блоке)
* catch		- обрабатывает исключение определенного типа данных (работает с try)
* 
* При выбрасывании исключения (throw), точка выполнения программы переходит к ближайшему
*	блоку try. Если какой-либо из обработчиков catch, прикрепленный к блоку try, обрабатывает этот
*	тип исключения, то точка выполнения переходит в этот обработчик, после выполненика блока кода
*	catch, исключение считается обработанным.
* 
* Если подходящих обработчиков catch 'не существует', то выполнение программы переходит в
*	следующий блок try. Если до конца программы не найдены соответствующие обработчики catch,
*	то программа завершает свое выполнение с ошибкой исключения.
* 
* Возможности catch:
*	1. Вывести сообщение об ошибке (в консоль/в лог-файл)
*	2. Возвратить код ошибки обратно в caller
*	3. Может сгенерировать другое исключения
* 
* try и раскручивание стека:
*	1. caller не обязан обрабатывать исключение, если он этого не может.
*	2. Если блок try не имеет нужного типа catch, то обрабатывается так, 
*		как если бы его вообще не было.
*	3. Когда исключение обработано, выполнение кода продолжается как обычно,
*		начиная с конца блока catch
* 
* 
* @.'Спецификации исключений' - механизм объявления функций с указанием того, будет ли функция
*		генерировать исключения или нет.
* 
*	!!! Не рекомендуется использовать вообще
*	(из-за плохой совместимости с компилятором, отсутствии гарантий, плохой совместимости
*		с шаблонами, и большинство программистов С++ не знают о их существовании)
* 
*	1. Не выбрасывать исключения
*	int doSomething() throw();
* 
*	2. Выбрасывать исключения с определенным типом
*	int doSomething() throw(double);
* 
*	3. Генерировать любые исключения
*	int doSomething() throw(...);
* 
* 
* 'Классы-Исключения и Наследование'
* 
*	! Обработчики могут обрабатывать исключения класса и дочерних ему в том числе
* 
* @.'Класс-Исключение' - Обычный класс, коотрый выбрасывается в качестве исключения
* 
*	! Исключение в конструкторе может прервать создание объекта, не вызывая деструктор 
*		(самостоятельно выполнить очистку памяти)
*	! noexcept (C++11) спецификатор, который означает, что функция обещает не выбрасывать
*		исключение самостоятельно
* 
* Применение:
*	Чтобы обрабатывать исключения по-разному
* 
* 'Написать самому'
* Пример
* class ArrayException // класс-исключение
* {
* private:
*	std::string m_error;
*
...
* 			throw ArrayException("InvalidIndex");	// бросаем исключение
* 
* 		return m_data[index];
* 	}
* };
...
* int main() {
* 	ArrayInt array;
* 	try { int value = array[7]; }
* 	catch (ArrayException &e) { std::cerr << e.getError() << '\n'; }
* }
* 
* 
* 'Использовать готовое решение'
* Эти классы-исключения существуют и в Стандартной Библиотеке С++.
* Все они являются дочерними классу std::exception (интерфейс)
* #include <exception>
* 
*	! Список стандартных исключений: https://ru.cppreference.com/w/cpp/error/exception
* 
* Пример
* int main() {
*	try
*	{
*		std::string s;
*		s.resize(-1); // исключение bad_alloc
*	}
*	catch (std::exception &e) // ловит std::exception и все дочерние ему классы-исключения
*	{
*		std::cerr << e.what() << '\n'; // what() возвращает строку C-style с описанием исключения
*	}
* }
* 
* Или ловить определенное исключение
*	'Популярный выбор' std::runtime_error имеющий общее имя. 
*	Принимает в конструктор настраиваемое сообщение.
* Пример
* catch (std::bad_alloc &e) { // ловит std::bad_alloc
...
* 
* 
* 'Наследовать готовое решение'
* Пример используя самописный ArrayException 
...
* Для повторной генерации одного и того же исключения нужно использовать
*	ключевое слово throw внутри блока catch без указания какого-либо идентификатора.
* Пример
* class Parent
* {
* public:
...
* public:
* 	Child() {}
* 	virtual void print() { std::cout << "Child" << '\a' << '\n'; }
* };
* 
* int main() {
* 	try {
...
* 			p.print();
* 			// throw p; - обрезка объекта класса Child!
*			throw; // бросаем повторное исключение
* 		}
* 	}
* 	catch (Parent &p) {
* 		std::cout << "Caught Parent p, which is actually a ";
...
* } 
* 
* !.'Правило:' При повторное генерации исключения используйте ключевое слово throw
*		без указания какого-либо идентификатора
* 
* 
* Что если класс Parent не создался, а обработать исключение хотим в Child?
* 'Функциональный try-блок'
* 
*	! Не используйте для очистки памяти
* 
* Пример
* class A
* {
